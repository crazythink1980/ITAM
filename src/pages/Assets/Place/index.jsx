...
class Place extends Component {
  ...
  columns = [
    ...
    },
    {
      ...
      render: (place) => {
        return (
          <span>
            <LinkButton onClick={() => this.showUpdate(place)}>
              修改分类
            </LinkButton>
            {place.hasChildren ? (
              <LinkButton onClick={() => this.displaySubPlaces(place)}>
                查看子分类
              </LinkButton>
            ) : (
              <LinkButton onClick={() => this.delPlace(place)}>
                删除分类
              </LinkButton>
            )}
          </span>
  ...
  render() {
    ...
    const title = parents.map((p, index) => {
      //最后一个元素
      if (index === parents.length - 1) {
        ...
        return (
          <>
            <LinkButton onClick={() => this.jumpToPlaces(index)}>
              {p.name}
            </LinkButton>
            <ArrowRightOutlined />
          </>
